...
```bash
git grep -in -F 'FUSE-Web' || echo "clean"
git grep -in -F 'fuse-web' || echo "clean"
git grep -in -F 'fuse_web' || echo "clean"
git grep -in -F 'Shield-Restoration-Services' || echo "clean"
git grep -in -F 'Collinson' || echo "clean"
git grep -in -F 'insurance catastrophe management' || echo "clean"
```
